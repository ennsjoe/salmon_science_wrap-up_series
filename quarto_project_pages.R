...
# ðŸ§¼ Helper functions (define early!)----
...
# ðŸ—‚ï¸ Define and validate database pat----
...
con <- tryCatch({
  ...
  stop(glue("âŒ Failed to connect to database: {e$message}"))
})

# Test connection before proceeding
if (!dbIsValid(con)) {
  stop("âŒ Database connection is not valid")
}

# ðŸ“Œ Validate required tables
cat("ðŸ“‹ Validating required tables...\n")
...
# ðŸ”— Initial join to create session_projects----
...
# ðŸŽ¯ Filter to speaker series projects only----
...
# ðŸ”Œ DISCONNECT NOW (we're done with the database)
dbDisconnect(con)

# ðŸ“‚ Create output directory----
cat("ðŸ“‚ Creating output directories...\n")
...
# ðŸ“ Generate .qmd pages----
cat("ðŸ“ Generating project pages...\n")
progress_count <- 0
...
# ðŸ“… Build December 2025 calendar
cat("ðŸ“… Building December 2025 calendar...\n")
...
# ðŸŽ¨ Single color for all sessions
...
# ðŸ“„ Generate index.qmd
cat("ðŸ“„ Generating index.qmd...\n")
...
for (date_key in names(presentations_by_date)) {
  ...
  index_md <- c(index_md, glue("## ðŸ“… {formatted_date}"), "")
  ...
  for (group in sessions_list) {
    ...
    index_md <- c(index_md, glue("### ðŸŸ {session_title}"), desc_text, "")
    ...
    projects_display <- group %>%
      ...
      mutate(
        ...
        source_emoji = case_when(
          source_program == "BCSRIF" ~ "ðŸŒ±",
          source_program == "PSSI" ~ "ðŸŒŠ",
          TRUE ~ "â“"
        )
      ) %>%
      distinct(project_id, .keep_all = TRUE) %>%
